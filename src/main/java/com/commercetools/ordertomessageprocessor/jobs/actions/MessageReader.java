...
public class MessageReader implements ItemReader<OrderCreatedMessage> {
    ...
    @Override
    public OrderCreatedMessage read() {
        if(isQueryNeeded()) {
            queryPlatform();
        }
    ...
    private OrderCreatedMessage getMessageFromList() {
        if (messages.isEmpty()){
            return null;
        }
        else{
            timeStampManager.setActualProcessedMessageTimeStamp(messages.get(0).getLastModifiedAt());
            return messages.remove(0).as(OrderCreatedMessage.class);
        }
    }
